...
ghost predicate DelegationMapComplete(dm:DelegationMap)
{
    forall k :: k in dm
}

ghost function {:opaque} UpdateDelegationMap(dm:DelegationMap, newkr:KeyRange, host:NodeIdentity) : DelegationMap
...
ghost predicate DelegateForKeyRangeIsHost(dm:DelegationMap, kr:KeyRange, id:NodeIdentity)
    requires DelegationMapComplete(dm);
{
...
ghost predicate DelegationMapsAreEqualUpToKey(adm:DelegationMap, bdm:DelegationMap, khi:KeyPlus)
...
// legacy definition
ghost function DelegateForKey(dm:DelegationMap, k:Key) : NodeIdentity
    requires DelegationMapComplete(dm);
{
    dm[k]
}
...
//////////////////////////////////////////////////////////////////////////////
// ghost functions to update a node's hash table on receipt and transmission of
// delegation messages:
//////////////////////////////////////////////////////////////////////////////

ghost function BulkUpdateDomain(h:Hashtable, kr:KeyRange, u:Hashtable) : set<Key>
{
...
ghost function BulkUpdateHashtable(h:Hashtable, kr:KeyRange, u:Hashtable) : Hashtable
{
    map k {:auto_trigger} | k in BulkUpdateDomain(h, kr, u) :: if (k in u) then u[k] else h[k]
}

ghost function BulkRemoveHashtable(h:Hashtable, kr:KeyRange) : Hashtable {
    map k | (k in h && !KeyRangeContains(kr, KeyPlus(k))) :: h[k]
}

ghost function ExtractRange(h:Hashtable, kr:KeyRange) : Hashtable {
    map k | (k in h && KeyRangeContains(kr, KeyPlus(k))) :: h[k]
}
