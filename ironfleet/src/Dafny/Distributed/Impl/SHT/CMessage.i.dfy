...
ghost predicate CMessageIsAbstractable(cmsg:CMessage)
{
       (cmsg.CRedirect? ==> EndPointIsAbstractable(cmsg.id))
    && (cmsg.CShard?    ==> EndPointIsAbstractable(cmsg.recipient))
}

ghost function AbstractifyCMessageToRslMessage(cmsg:CMessage) : Message
...
ghost predicate CSingleMessageIsAbstractable(smsg:CSingleMessage)
{
    match smsg
...
ghost function AbstractifyCSingleMessageToSingleMessage(smsg:CSingleMessage) : SingleMessage<Message>
...
ghost predicate CSingleMessageIsValid(smsg:CSingleMessage, params:CParameters)
{
    match smsg
...
ghost predicate CSingleMessageSeqIsAbstractable(cs:seq<CSingleMessage>) 
{
    forall i :: 0 <= i < |cs| ==> CSingleMessageIsAbstractable(cs[i])
}

ghost function AbstractifySeqOfCSingleMessageToSeqOfSingleMessage(cs:seq<CSingleMessage>) : seq<SingleMessage<Message>>
...
ghost predicate CPacketIsAbstractable(cpacket:CPacket)
{
...
ghost function AbstractifyCPacketToShtPacket(cpacket:CPacket) : Packet
...
ghost function {:opaque} AbstractifyCPacketsToPackets(cps:set<CPacket>) : set<Packet>
...
ghost function {:opaque} AbstractifySeqOfCPacketsToSetOfShtPackets(cps:seq<CPacket>) : set<Packet>
...
ghost predicate OptionCPacketIsAbstractable(cp:Option<CPacket>)
{
    match cp {
...
ghost function AbstractifyOptionCPacketToOptionShtPacket(cp:Option<CPacket>) : Option<Packet>
