...
ghost function stepmap(f:imap<int, bool>):temporal
  ensures  forall i:int :: i in f ==> sat(i, stepmap(f)) == f[i]
{
  f
}

ghost predicate sat(s:int, t:temporal)
{
  s in t && t[s]
}
