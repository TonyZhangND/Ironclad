...
static ghost function {:opaque} Ch(x: int, y: int, z: int) : int
    requires Word32(x);
    requires Word32(y);
    requires Word32(z);
...
static ghost function {:opaque} Maj(x: int, y: int, z: int) : int
    requires Word32(x);
    requires Word32(y);
    requires Word32(z);
...
static ghost function {:opaque} Parity(x: int, y: int, z: int) : int
    requires Word32(x);
    requires Word32(y);
    requires Word32(z);
...
static ghost function {:opaque} ft(t: int, x: int, y: int, z: int) : int
    requires 0 <= t <= 79;
    requires Word32(x);
    requires Word32(y);
    requires Word32(z);
...
static ghost function {:opaque} BSIG0(x: int) : int
    requires Word32(x);
...
static ghost function {:opaque} BSIG1(x: int) : int
    requires Word32(x);
...
static ghost function {:opaque} SSIG0(x: int) : int
    requires Word32(x);
...
static ghost function {:opaque} SSIG1(x: int) : int
    requires Word32(x);
...
static function {:opaque} NumPaddingZeroes(message_len: int) : int
...
static ghost function PadMessageForSHA(messageBits: seq<int>) : seq<int>
{
    messageBits + [1] + RepeatDigit(0, NumPaddingZeroes(|messageBits|)) + BEIntToDigitSeq(2, 64, |messageBits|)
}

static ghost function {:opaque} BlockMessageForSHA(paddedMessageBits: seq<int>) : seq<seq<int>>
...
static function OneOf8(i: int, n0: int, n1: int, n2: int, n3: int, n4: int, n5: int, n6: int, n7: int) : int
    requires 0 <= i < 8;
{
...
static ghost function TBlk(blk:int):bool { true }
static ghost function TStep(t:int):bool { true }
