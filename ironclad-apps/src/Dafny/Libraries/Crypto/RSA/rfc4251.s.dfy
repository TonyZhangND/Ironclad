...
static ghost function {:autoReq} rfc4251_word32_encoding(w:int) : seq<int>
{
    BEWordSeqToByteSeq([w])
}

static ghost function {:autoReq} rfc4251_string_encoding(s:seq<int>) : seq<int>
...
static ghost predicate rfc4251_positive_twoscomplement(s:seq<int>)
{
    (|s|==0
        || 0<s[0]<128
        || (s[0]==0 && |s|>1 && s[1]>=128))
...
static ghost function rfc4251_positive_to_twoscomplement(s:seq<int>) : seq<int>
{
    if |s|==0 || s[0]<128 then
        s
    else
...
static ghost function {:autoReq} rfc4251_mpint_encoding(v:nat) : seq<int>
{
    var tc := rfc4251_positive_to_twoscomplement(BEIntToByteSeq(v));
    rfc4251_word32_encoding(|tc|) + tc
}

static function STR_SSH_RSA() : seq<int>
...
static ghost function {:autoReq} rfc4251_sshrsa_encoding(e:nat, n:nat) : seq<int>
{
...
static ghost function {:autoReq} rfc4251_sshrsa_pubkey_encoding(key:RSAPubKeySpec) : seq<int>
