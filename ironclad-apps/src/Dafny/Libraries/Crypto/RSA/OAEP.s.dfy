...
static ghost predicate {:autoReq} OAEPEncryptionRelation(pubkey:RSAPubKeySpec, M:seq<int>, EM:seq<int>, old_TPM:TPM_struct, new_TPM:TPM_struct)
{
...
static ghost predicate {:autoReq} OAEPDecryptionRelation(key:RSAKeyPairSpec, C:seq<int>, decryption_result:OAEPDecryptionResult)
{
...
static ghost function {:autoReq} SHA256_BytesToBytes(M:seq<int>) : seq<int>
    requires IsByteSeq(M);
{
    BEWordSeqToByteSeq(SHA256(BEByteSeqToBitSeq(M)))
}

static ghost function ByteSeqXor(X:seq<int>, Y:seq<int>) : seq<int>
...
static ghost function FindFirstNonzeroByte(s:seq<int>, minpos:nat) : int
...
static ghost function hLen() : int
{
    32  
}

static ghost function sLen() : int
{
    hLen()
}
...
static ghost function I2OSP(x:int, xLen:int) : seq<int>
    requires 0 <= xLen;
...
static ghost function OS2IP(X:seq<int>) : int
    requires IsByteSeq(X);
{
    BEByteSeqToInt(X)
}
...
static ghost function RSAEP(pubkey:RSAPubKeySpec, m:int) : int
...
static ghost function RSADP(keypair:RSAKeyPairSpec, c:int) : int
...
static ghost function RSASP1(key:RSAKeyPairSpec, m:int) : int
...
static ghost predicate MGF1_step_requirements(seed:seq<int>, counter:nat)
{
...
static ghost function MGF1_step(seed:seq<int>, counter:nat) : seq<int>
...
static ghost function MGF1_prefix(seed:seq<int>, counter:nat) : seq<int>
...
static ghost function MGF1(seed:seq<int>, maskLen:int) : seq<int>
...
static ghost function {:autoReq} RSAES_OAEP_ENCRYPT(pubkey:RSAPubKeySpec, M:seq<int>, L:seq<int>, seed:seq<int>) : seq<int>
    requires WellformedRSAPubKeySpec(pubkey);
    requires IsByteSeq(M);
...
static ghost function {:autoReq} RSAES_OAEP_DECRYPT(key:RSAKeyPairSpec, C:seq<int>, L:seq<int>) : OAEPDecryptionResult
    requires WellformedRSAKeyPairSpec(key);
