...
static ghost function UndefinedBit(index:int, val:int):bool
static ghost function IntBit_spec(index:int, val:int):bool
{
...
static ghost function{:axiom} BitwiseAnd(x:int, y:int) : int
    requires Word32(x);
    requires Word32(y);
...
static ghost function{:axiom} BitwiseOr(x:int, y:int):int
    requires Word32(x);
    requires Word32(y);
...
static ghost function{:axiom} BitwiseNot(x:int) : int
    requires Word32(x);
...
static ghost function{:axiom} BitwiseXor(x:int, y:int) : int
    requires Word32(x);
    requires Word32(y);
...
static ghost function{:axiom} RotateRight(x:int, amount:int) : int
    requires Word32(x);
...
static ghost function{:axiom} RotateLeft(x:int, amount:int):int
    requires Word32(x);
    requires 0 <= amount < 32;
...
static ghost function{:axiom} RightShift(x:int, amount:int) : int
    requires Word32(x);
    requires 0 <= amount < 32;
...
static ghost function{:axiom} LeftShift(x:int, amount:int):int
    requires Word32(x);
    requires 0 <= amount < 32;
...
static ghost function{:axiom} Add32(x:int, y:int):int
    requires Word32(x);
    requires Word32(y);
    ensures Word32(Add32(x, y));
    ensures Add32(x, y) == (x + y) % 0x100000000;

static ghost function{:axiom} Sub32(x:int, y:int):int
    requires Word32(x);
    requires Word32(y);
    ensures Word32(Sub32(x, y));
    ensures Sub32(x, y) == (x - y) % 0x100000000;

static ghost function{:axiom} Mul32(x:int, y:int):int
    requires Word32(x);
    requires Word32(y);
    ensures Word32(Mul32(x, y));
    ensures Mul32(x, y) == (x * y) % 0x100000000;

static ghost function{:axiom} Div32(x:int, y:int):int
    requires Word32(x);
    requires Word32(y);
    requires y != 0;
...
static ghost function{:axiom} Mod32(x:int, y:int):int
    requires Word32(x);
    requires Word32(y);
    requires y != 0;
...
/*
static ghost function BitwiseNot(x:int) : int
    requires Word32(x);
    //-ensures Word32(BitwiseNot(x));
{
...
static ghost function BitwiseNot_helper(x:int, bits:int) : int
{
    if bits <= 0 then 0
    else 2*BitwiseNot_helper(x, bits - 1) + (1 - (x % 2))
}

static ghost function BitwiseAnd(x:int, y:int) : int
    requires Word32(x);
    requires Word32(y);
    //-ensures Word32(BitwiseAnd(x, y));
{
...
static ghost function BitwiseAnd_helper(x:int, y:int, bits:int) : int
{
    if bits <= 0 then 0
...
static ghost function BitwiseNot_helper(x:int, bits:int) : seq<int>
{
    if bits <= 0 then []
...
static ghost function BitwiseXor(x:int, y:int) : int
    requires Word32(x);
    requires Word32(y);
    ensures Word32(BitwiseXor(x, y));
{
...
static ghost function RotateRight(x:int, amount:int) : int
    requires Word32(x);
    requires 0 <= amount < 32;
    ensures Word32(RotateRight(x, amount));
{
...
static ghost function RotateLeft(x:int, amount:int):int
    requires Word32(x);
    requires 0 <= amount < 32;
    ensures Word32(RotateLeft(x, amount));
{
...
static ghost function RightShift(x:int, amount:int) : int
    requires Word32(x);
    requires 0 <= amount < 32;
    ensures Word32(RightShift(x, amount));
{
...
static ghost function LeftShift(x:int, amount:int):int
    requires Word32(x);
    requires 0 <= amount < 32;
    ensures Word32(LeftShift(x, amount));
{
...
static ghost function Add32(x:int, y:int):int
    requires Word32(x);
    requires Word32(y);
    ensures Word32(Add32(x, y));
{
...
static ghost function{:imported} IntBit_spec(index:int, n:int):bool
...
static function{:imported} asm_Add(x:int, y:int):int
    requires word32(x);
    requires word32(y);
    ensures word32(asm_Add(x, y));
    ensures asm_Add(x, y) == mod0x100000000(x + y);

static function{:imported} asm_Sub(x:int, y:int):int
    requires word32(x);
    requires word32(y);
...
static function{:imported} asm_LeftShift(x:int, amount:int):int
    requires word32(x);
    requires 0 <= amount < 32;
...
static function{:imported} asm_RightShift(x:int, amount:int):int
    requires word32(x);
    requires 0 <= amount < 32;
...
static function{:imported} asm_RotateLeft(x:int, amount:int):int
    requires word32(x);
    requires 0 <= amount < 32;
...
static function{:imported} asm_RotateRight(x:int, amount:int):int
    requires word32(x);
    requires 0 <= amount < 32;
...
static function{:imported} asm_BitwiseNot(x:int):int
    requires word32(x);
...
static function{:imported} asm_BitwiseAnd(x:int, y:int):int
    requires word32(x);
    requires word32(y);
...
static function{:imported} asm_BitwiseOr(x:int, y:int):int
    requires word32(x);
    requires word32(y);
...
static function{:imported} asm_BitwiseXor(x:int, y:int):int
    requires word32(x);
    requires word32(y);
