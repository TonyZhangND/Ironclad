...
static ghost function RealPower(x:real, e:nat) : real
    decreases e;
{
    if e == 0 then 1.0 else x * RealPower(x, e-1)
}

static ghost function RealMin(a:real, b:real) : real
{
...
static ghost function DiffPrivParametersValid (p:DiffPrivParameters) : bool
{
    p.delta >= 0 &&
    p.alpha > 1.0 &&
...
static ghost function RequiredNoiseEntropy (p:DiffPrivParameters) : real
...
static ghost function {:opaque} ModuloFour (x:int) : int
{
    x % 4
}

static ghost function {:opaque} SufficientBytesForNoiseGeneration (p:DiffPrivParameters, bytes:nat) : bool
    requires DiffPrivParametersValid(p);
{
    bytes >= 1 &&
    ModuloFour(bytes) == 1 &&
...
static ghost function NoiseThreshold (alpha:real, u:real) : real
    requires u > 0.0;
    requires alpha > 1.0;
{
...
static ghost predicate {:opaque} ShouldNegateNoise (r:int)
{
    (r % 2 == 1)
}

static ghost function GetUniformBetweenZeroAndOne (randoms:seq<int>) : real
    requires IsByteSeq(randoms);
{
    (real(BEByteSeqToInt(randoms)) + 0.5) / real(power2(|randoms|*8))
}

static ghost function NoiseOK (p:DiffPrivParameters, negate_noise:bool, u:real, absolute_noise:nat, noise:int) : bool
    requires DiffPrivParametersValid(p);
{
    u > 0.0 &&
...
static ghost function AbsoluteNoiseComputedCorrectlyTrigger (absolute_noise:nat) : bool
{
    true
}

static ghost function NoiseComputedCorrectly (p:DiffPrivParameters, randoms:seq<int>, noise:int) : bool
    requires DiffPrivParametersValid(p);
{
    |randoms| > 0 &&
    IsByteSeq(randoms) &&
